...
- **Parâmetros**:
  ...
  - `message`: Texto da mensagem ou legenda. Suporta tags de interpolação como `{{trigger.payload.description}}` ou `{{trigger.payload.cameraName}}`.
...
1. **Alerta de Carro/Pessoa no WhatsApp**:
   ...
   - Ação: `whatsapp.send_message`
     ...
     - `message`: `"🚨 *Alerta MomAI Vision!*\n\nDetectado na câmera {{trigger.payload.cameraName}}: {{trigger.payload.description}}"`
