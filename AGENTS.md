...
2. Schemas, types, manifests, and registries: TypeScript interfaces, Pydantic models, `manifest.json`, `dev-extensions.json`.
...
- `dev-extensions.json` - Developer local extension catalog.
...
- If touching extension development under `apps/momai/data/extensions/<id>/` (or in its external repo), read the skill's `manifest.json` and `runtime.js`/`background-worker.js`.
...
### Decentralized Skill Layout

Extensões e skills de terceiros são descentralizadas e instaladas no diretório local de dados do usuário:
`data/extensions/<id>/` (em desenvolvimento, `apps/momai/data/extensions/<id>/`).

Para o fluxo de desenvolvimento de novas extensões, consulte a documentação detalhada em [docs/extension-development-guide.md](docs/extension-development-guide.md).
...
- `dev-extensions.json` at the repo root: local developer allowlist override used to validate install IDs and download URLs during development.
...
- Install requests must match the allowlist entry in `dev-extensions.json` when using the local install registry in dev.
...
### Decentralized Extensions and Repositories

Todas as extensões opcionais e utilitárias (como WhatsApp e Launcher) vivem exclusivamente em seus respectivos repositórios no GitHub:
- `WesleyQDev/momai-open` (Launcher)
- `WesleyQDev/momai-whatsapp-extension` (WhatsApp)

Elas são mantidas em código aberto e não devem ter pastas locais no monorepo principal do MomAI. Para testes e desenvolvimento, use a técnica de **symlinks** apontando para `data/extensions/` como detalhado em `docs/extension-development-guide.md`.
