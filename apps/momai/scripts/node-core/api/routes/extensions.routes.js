 ...
 *                real directory in `<extensionsDir>/<extId>`
 *
 ...
 * it is a real directory (not a symlink) left over from store_test mode.
 */
function cleanupOppositeModeArtifact(extensionsDir, extensionsDevDir, extId, devMode) {
  ...
  try {
    ...
    if (devMode === 'symlink') {
      ...
      // store_test mode: `.dev/<extId>` should not exist. Remove symlinks
      // generated by ensureDevSymlink and real directories left over from
      // symlink mode.
      if (lstat.isSymbolicLink()) {
