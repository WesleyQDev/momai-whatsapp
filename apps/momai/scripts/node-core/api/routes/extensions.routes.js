  ...
  if (devMode === 'symlink') {
    ...
    try {
      ...
      if (!lstat.isSymbolicLink()) {
        // Not a symlink — remove it (old directory leftover)
        fs.rmSync(devLink, { recursive: true, force: true })
      } else {
        // It's a symlink — only remove if it points inside the extensions dir
        // (auto-generated by ensureDevSymlink). External symlinks (user-created
        // local checkouts) must be preserved.
        const target = fs.realpathSync(devLink)
        const extDir = path.resolve(extensionsDir)
        if (target.startsWith(extDir)) {
          fs.unlinkSync(devLink)
        } else {
          console.log(
            `[extensions] Preserving external dev symlink for ${extId} → ${target}`
          )
        }
      }
    } catch {
