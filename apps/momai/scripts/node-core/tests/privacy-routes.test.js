...
describe('privacy route', () => {
  ...
  test('POST /privacy/delete-all with correct confirmation removes expected files', async () => {
    ...
    fs.mkdirSync(path.join(tmpDir, 'extensions', 'whatsapp'), { recursive: true })
    fs.writeFileSync(path.join(tmpDir, 'extensions', 'whatsapp', 'creds.json'), 'x')
    ...
    expect(fs.existsSync(path.join(tmpDir, 'extensions', 'whatsapp'))).toBe(false)
