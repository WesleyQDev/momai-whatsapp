                ...
                >
                  <div className="flex items-start gap-2.5">
                    <div className={`w-2 h-2 rounded-full mt-0.5 shrink-0 ${
                      ...
                      trace.type === 'skill' ? 'bg-emerald-400' : 'bg-blue-400'
                    }`} />
                    <div className="min-w-0 flex-1">
                      <p className="text-xs text-text truncate">{lastUserMsg(trace).slice(0, 60) || '(vazio)'}</p>
          ...
          {!selected ? (
            ...
            <div className="px-6 py-5 lg:max-w-4xl lg:mx-auto">
              ...
              {selected.messages && selected.messages.length > 0 && (
                <>
                  <div className="mb-4">
                    <div className="text-xs font-medium text-text-muted/50 mb-2">Mensagens ({selected.messages.length})</div>
                    <div className="space-y-1">
                      {selected.messages.map((msg, i) => (
                        <div key={i} className={`text-sm px-3 py-2 rounded-lg leading-relaxed ${
                          msg.role === 'user' ? 'bg-blue-500/5' :
                          msg.role === 'tool' ? 'bg-emerald-500/5' : ''
                        }`}>
                          <span className="text-[10px] font-semibold text-text-muted/40 uppercase mr-2">{msg.role}</span>
              ...
              {selected.tool_calls && selected.tool_calls.length > 0 && (
                <>
                  <div className="mb-4">
                    <div className="text-xs font-medium text-text-muted/50 mb-2">Tools ({selected.tool_calls.length})</div>
                      ...
                      {selected.tool_calls.map((tc, i) => (
                        <div key={i} className="border border-border/8 rounded-lg overflow-hidden">
                          <div className="flex items-center gap-2 px-3 py-2 bg-white/[0.02] border-b border-border/8">
                            <svg className="w-3.5 h-3.5 text-accent shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z" /></svg>
                            <span className="text-xs font-semibold text-accent">{tc.tool_name}</span>
                            {tc.duration_ms != null && (
                              <span className="ml-auto text-[10px] text-text-muted/50 tabular-nums">{formatDuration(tc.duration_ms)}</span>
                            )}
                          </div>
                          {tc.args && Object.keys(tc.args).length > 0 && (
                            <div className="px-3 py-2 border-b border-border/5">
                              <div className="text-[10px] text-text-muted/40 mb-0.5">Args</div>
                              <pre className="text-xs text-text-muted/70 font-mono">{JSON.stringify(tc.args)}</pre>
                            </div>
                          )}
                          {tc.result && (
                            <div className="px-3 py-2">
                              <div className="text-[10px] text-text-muted/40 mb-0.5">Resultado</div>
                              <pre className="text-xs text-text-muted/70 font-mono whitespace-pre-wrap max-h-20 overflow-y-auto">{tc.result}</pre>
                            </div>
                          )}
                        </div>
                      ))}
              ...
              {selected.status === 'error' && (
                <>
                  <div className="mb-4 px-3 py-2 bg-red-500/5 border border-red-500/15 rounded-lg">
                    ...
                    <p className="text-sm text-red-300/90">{selected.error || 'Erro desconhecido'}</p>
                  </div>
                  <Divider />
                </>
              )}
                    ...
                    {[
                      ...
                      value != null ? (
                        <div key={label as string} className="flex items-center justify-between">
                          ...
                          <span className="text-xs text-text font-mono tabular-nums">{formatDuration(value as number)}</span>
                        </div>
                      ) : null
                    )}
                    {selected.tool_calls && selected.tool_calls.length > 0 && (
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs text-text font-mono tabular-nums">{formatDuration(totalToolTime(selected))}</span>
                      </div>
                    )}
                    <div className="flex items-center justify-between pt-1 mt-1 border-t border-border/8">
                      <span className="text-xs text-text-muted">Total</span>
                      <span className="text-xs text-text font-mono tabular-nums font-semibold">{formatDuration(selected.total_duration)}</span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-1">
                    ...
                    {selected.tokens_per_second > 0 && (
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs text-text font-mono tabular-nums">{selected.tokens_per_second.toFixed(1)} tok/s</span>
                      </div>
                    )}
                    <div className="flex items-center justify-between pt-1 mt-1 border-t border-border/8">
                      <span className="text-xs text-text-muted">Total</span>
                      <span className="text-xs text-text font-mono tabular-nums font-semibold">{selected.total_tokens}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-1.5 text-[10px] text-text-muted/30 pb-4">
                ...
                {selected.thread_id && <><span>·</span><span>#{selected.thread_id.slice(0, 8)}</span></>}
              </div>
            </div>
          )}
        </div>
