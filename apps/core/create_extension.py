...
logger = logging.getLogger("momai.cli")
...
def create_extension(name: str):
    ...
    if base_path.exists():
        logger.error(f"Error: Extension {ext_id} already exists at {base_path}")
        return
...
logger = logging.getLogger("momai.skill.{ext_id}")
...
class {name.replace(" ", "")}Plugin:
    ...
    def on_startup(self):
        ...
        logger.info(f"[{name}] Inicializada!")
...
"""
    ...
    logger.info(f"Extension {name} created successfully at {base_path}")
    logger.info(f"ID: com.momai.extension.{ext_id}")

if __name__ == "__main__":
    if len(sys.argv) < 2:
        logger.info("Usage: python create_extension.py 'My Extension Name'")
    else:
