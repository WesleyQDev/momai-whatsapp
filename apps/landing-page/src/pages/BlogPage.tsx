  ...
  return (
    <div className="blog-post-container">
      <div className="mx-auto max-w-[760px] px-6 py-16 md:py-24">
        ...
        <div className="mb-10">
          {post.featured && (
            <span className="mb-4 inline-block rounded-full bg-[var(--gradient-primary)] px-4 py-1 text-xs font-semibold uppercase tracking-wider text-white">
              {t("blog.novidade")}
            </span>
          )}
          ...
          <h1 className="mb-5 font-flex text-3xl font-normal leading-[1.3] tracking-tight text-[var(--text)] md:text-4xl">
            {post.title}
          </h1>
          {post.author && (
    ...
    <div className="blog-listing-container">
      <div className="mx-auto max-w-[1100px] px-8 py-24">
        <div className="mb-16 text-center">
          <h1
            className="mb-3 font-flex text-5xl font-normal tracking-tight"
            style={{
        ...
        {heroPost && (
          ...
          >
            ...
            <div className="flex w-1/2 flex-col justify-center p-10 max-md:w-full max-md:p-6">
              <span className="mb-3 inline-block w-fit rounded bg-[var(--gradient-primary)] px-3 py-1 text-xs font-semibold uppercase tracking-wider text-white">
                {t("blog.featured")}
              </span>
              <div className="mb-2 text-xs font-medium uppercase tracking-wider text-[var(--text-tertiary)]">
                {heroPost.date}
              </div>
              <h2 className="mb-3 font-flex text-3xl font-normal leading-[1.3] text-[var(--text)]">
                {heroPost.title}
              </h2>
              <p className="mb-4 line-clamp-4 text-base leading-relaxed text-[var(--text-secondary)]">
                {heroPost.excerpt}
              </p>
          ...
          {[...remainingFeatured, ...regularPosts].map((post) => (
            ...
            >
              ...
              <div className="flex flex-col p-6">
                {post.featured && (
                  <span className="mb-2 inline-block w-fit rounded bg-[var(--gradient-primary)] px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-white">
                    {t("blog.featured")}
                  </span>
                )}
                ...
                <h3 className="mb-2 text-lg font-medium leading-[1.4] text-[var(--text)]">
                  {post.title}
                </h3>
                <p className="mb-4 line-clamp-3 flex-1 text-sm leading-relaxed text-[var(--text-secondary)]">
                  {post.excerpt}
                </p>
                <div className="text-sm font-medium text-[var(--accent)]">
                  {t("blog.leiaMais")}
        ...
        {posts.length === 0 && (
          <p className="py-16 text-center text-[var(--text-tertiary)]">
            {t("blog.empty")}
          </p>
        )}
      </div>
...
export function BlogPage() {
  ...
  if (selectedPost) {
    return (
      <BlogPostView post={selectedPost} onBack={() => navigate("/blog")} />
    );
  }
