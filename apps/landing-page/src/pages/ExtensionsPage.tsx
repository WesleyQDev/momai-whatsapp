...
import { ExtensionCard, getIcon, getGradient } from '../components/ExtensionCard'
...
export function ExtensionsPage() {
  ...
  useEffect(() => {
    ...
    fetch(`https://raw.githubusercontent.com/${selectedExtension.repo}/main/README.md`)
      .then((r) => {
        if (!r.ok) throw new Error()
        return r.text()
      })
    ...
    return (
      <div className="relative mx-auto max-w-3xl px-6 py-16 sm:px-8 lg:px-12">
        <div className="pointer-events-none fixed inset-0 overflow-hidden" aria-hidden>
          <div className={`absolute -left-40 top-40 h-96 w-96 rounded-full opacity-10 blur-[120px] ${gradClass.replace('from-', 'bg-').replace('to-', '').split(' ')[0]}`} />
        </div>
        ...
        >
          <svg className="h-4 w-4 transition-transform duration-300 group-hover:-translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
          {t('extensoes.voltar')}
        </button>

        <div className="mb-14 flex flex-col gap-8 sm:flex-row sm:items-start">
          <div className={`relative flex h-28 w-28 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br ${gradClass} shadow-2xl shadow-black/30`}>
          ...
          <div className="flex-1">
            ...
            <div className="mb-7 flex flex-wrap items-center gap-8">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="block text-[10px] font-semibold uppercase tracking-wider text-[var(--text-tertiary)]">{t('extensoes.autor')}</span>
                  <span className="text-sm font-medium text-[var(--text)]">{selectedExtension.author}</span>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <div className="flex h-9 w-9 items-center justify-center rounded-lg border border-[var(--feature-border)] bg-[var(--bg-tertiary)]">
                  <svg className="h-4 w-4 text-[var(--text-secondary)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                </div>
                <div>
                  <span className="block text-[10px] font-semibold uppercase tracking-wider text-[var(--text-tertiary)]">{t('extensoes.versao')}</span>
                  <span className="text-sm font-medium text-[var(--text)]">v{selectedExtension.version}</span>
                </div>
              </div>
            </div>
        ...
        <div className="relative">
          ...
          <div className="pt-10">
            <h2 className="mb-6 flex items-center gap-2 text-sm font-semibold uppercase tracking-wider text-[var(--text-secondary)]">
              <svg className="h-4 w-4 text-[var(--accent)]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              {t('extensoes.sobre')}
            </h2>
            <div className="rounded-2xl border border-[var(--feature-border)] bg-[var(--bg-tertiary)]/60 p-8 backdrop-blur-xl">
              ...
              {readmeError && (
                <p className="text-center text-sm text-[var(--text-tertiary)]">{t('extensoes.readmeError')}</p>
              )}
              {readme && !readmeLoading && (
                <div
                  ...
                  dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(marked.parse(readme) as string) }}
                />
              )}
              {!readme && !readmeLoading && !readmeError && (
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-2xl border border-[var(--feature-border)] bg-[var(--bg-secondary)]">
                    <svg className="h-8 w-8 text-[var(--text-tertiary)]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                  </div>
                  <p className="text-sm italic text-[var(--text-tertiary)]">{t('extensoes.readmeEmpty')}</p>
                </div>
              )}
            </div>
          </div>
    ...
    <div className="relative mx-auto max-w-5xl px-6 py-24 sm:px-8 lg:px-12">
      ...
      <div className="relative mb-16 text-center">
        <div className="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-[var(--accent)]/20 to-transparent ring-1 ring-[var(--accent)]/10 backdrop-blur-xl">
          <svg className="h-7 w-7 text-[var(--accent)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M11.42 15.17l-5.384 5.384a2.25 2.25 0 01-3.182-3.182l5.384-5.384m3.182 3.182a11.19 11.19 0 01-3.182-7.87m3.182 7.87a11.19 11.19 0 007.87-3.182m-7.87 3.182l7.87-7.87m-7.87 7.87a11.19 11.19 0 01-3.182-7.87m3.182 7.87a11.19 11.19 0 007.87-3.182" /></svg>
        </div>
        <h1 className="mb-4 text-4xl font-bold tracking-tight text-[var(--text)] sm:text-5xl">{t('extensoes.title')}</h1>
      ...
      {error && (
        <p className="py-20 text-center text-[var(--text-tertiary)]">{t('extensoes.error')}</p>
      )}
      {!loading && !error && extensions.length === 0 && (
        <p className="py-20 text-center text-[var(--text-tertiary)]">{t('extensoes.empty')}</p>
      )}
