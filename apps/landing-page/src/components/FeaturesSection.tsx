...
const FEATURES = [
  {
    icon: MicrophoneIcon,
    title: <>Diga <span className="font-semibold text-[var(--accent)] drop-shadow-[0_0_20px_rgba(var(--accent-rgb),0.4)]">&quot;Luna&quot;</span></>,
    description: 'Chame sua assistente a qualquer momento. Wake word integrada, sem precisar clicar em nada.',
    highlight: true,
  },
  {
    icon: ShieldIcon,
    title: 'Privacidade Total',
    description: 'Seus dados nunca saem do seu computador. Nenhuma informação é enviada para servidores externos.',
  },
  {
    icon: ZapIcon,
    title: 'IA 100% Local',
    description: 'Funciona completamente offline. Sem depender de APIs, serviços na nuvem ou conexão com a internet.',
  },
  {
    icon: PuzzleIcon,
    title: 'Extensível',
    description: 'Instale extensões para expandir suas capacidades — desde buscas na web até automações do sistema.',
  },
  {
    icon: ClockIcon,
    title: 'Lembretes Inteligentes',
    description: 'Crie lembretes e agende tarefas com linguagem natural. A MomAI avisa você no momento certo.',
  },
  {
    icon: GlobeIcon,
    title: 'Busca na Web',
    description: 'Quando precisa de informações atualizadas, a MomAI busca na web e traz os resultados para você.',
  },
]
  ...
  return (
    <section id="features" className="features-section relative bg-[var(--bg-secondary)] px-8 py-24">
      <div className="section-title mx-auto mb-16 max-w-[480px] text-center">
        <h2 className="mb-3 font-flex text-3xl font-normal tracking-tight text-[var(--accent)]">Por que escolher a MomAI?</h2>
        <p className="text-base text-[var(--text-secondary)]">Uma assistente projetada para quem valoriza privacidade, controle e simplicidade.</p>
      </div>
        ...
        {FEATURES.map((feature, i) => (
          <ScrollReveal key={i} delay={i * 0.1}>
            ...
            >
              ...
              <div className={`feature-icon relative z-[1] mb-5 flex h-11 w-11 items-center justify-center rounded-xl border bg-[var(--gradient-subtle)] ${feature.highlight ? 'border-[rgba(var(--accent-rgb),0.3)] bg-[rgba(var(--accent-rgb),0.15)]' : 'border-[rgba(138,180,248,0.15)]'}`}>
                <feature.icon className={`h-[22px] w-[22px] stroke-[var(--accent)] ${feature.highlight ? 'drop-shadow-[0_0_8px_rgba(var(--accent-rgb),0.5)]' : ''}`} />
              </div>

              <h3 className="relative z-[1] mb-2 text-base font-medium text-[var(--text)]">{feature.title}</h3>
              <p className="relative z-[1] text-sm leading-relaxed text-[var(--text-secondary)]">{feature.description}</p>
            </div>
          </ScrollReveal>
        ))}
      </div>
