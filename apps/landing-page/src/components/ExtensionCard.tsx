...
const ICON_MAP: Record<string, React.ReactNode> = {
  ...
  Wrench: (
    <svg className="h-6 w-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M11.42 15.17l-5.384 5.384a2.25 2.25 0 01-3.182-3.182l5.384-5.384m3.182 3.182a11.19 11.19 0 01-3.182-7.87m3.182 7.87a11.19 11.19 0 007.87-3.182m-7.87 3.182l7.87-7.87m-7.87 7.87a11.19 11.19 0 01-3.182-7.87m3.182 7.87a11.19 11.19 0 007.87-3.182" />
    </svg>
  ),
  PuzzlePiece: (
    <svg className="h-6 w-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
export function ExtensionCard({ name, description, category, author, version, icon, onClick }: ExtensionCardProps) {
  const gradClass = getGradient(name)
  return (
    <button
      onClick={onClick}
      className="group relative overflow-hidden rounded-2xl border border-[var(--feature-border)] bg-[var(--bg-tertiary)]/60 p-6 text-left backdrop-blur-xl transition-all duration-500 hover:-translate-y-1 hover:border-[var(--accent)]/30 hover:shadow-[0_0_30px_-5px_rgba(var(--accent-rgb),0.15)] active:scale-[0.98]"
    >
      <div className="pointer-events-none absolute -inset-px rounded-2xl opacity-0 transition-opacity duration-500 group-hover:opacity-100" style={{
        background: `radial-gradient(600px circle at var(--mouse-x, 50%) var(--mouse-y, 50%), rgba(var(--accent-rgb), 0.04), transparent 40%)`
      }} />

      <div className="relative">
        <div className="mb-5 flex items-start justify-between gap-4">
          <div className={`relative flex h-14 w-14 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br ${gradClass} shadow-lg shadow-black/20`}>
